 ...
 * @return 返回转换后的日期。
 */
fun String?.parseDate(format: String = DATE_FORMAT): Date? {
 ...
 * 判断年月是否相同
 */
fun Date.isSameYearMonth(date: Date?): Boolean {
    date ?: return false
    val calendar1 = Calendar.getInstance().apply { time = this@isSameYearMonth }
    val calendar2 = Calendar.getInstance().apply { time = date }
    return calendar1.get(Calendar.ERA) == calendar2.get(Calendar.ERA) && calendar1.get(Calendar.YEAR) == calendar2.get(
        Calendar.YEAR
    ) && calendar1.get(
...
fun daysBetween(startDate: Date, endDate: Date): Int {
    ...
    val entTime = Calendar.getInstance().run {
        time = endDate
        set(Calendar.HOUR_OF_DAY, 0)
        set(Calendar.MINUTE, 0)
        set(Calendar.SECOND, 0)
    ...
    return (abs(startTime - entTime) / (24 * 60 * 60 * 1000)).toInt()
}
 ...
 * 获取当前日期，年月日显示
 */
fun getNowDateString(): String = Date().format(DATE_FORMAT)
